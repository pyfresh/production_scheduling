...
Schedule GreedySearch(const Schedule &schedule, const std::vector<Job> &jobs, const std::vector<std::string> &jobList, const int iter_count) {
    ...
    std::vector<std::string> processlist1 = ScoreProcessForExchangeOld(schedule, jobs, jobList);
    ...
    while (scheduleList.size() < iter_count && (processlist1.size()>0 || processlist2.size()> 0)) {
        ...
        if (processlist1.size() > 0) {
            temp_schedule = ExchangeNeighborSearchOld(schedule, jobs, jobList, processlist1, flag1);
            if (temp_schedule.get_schedule_id()!=-1) scheduleList.push_back(temp_schedule);
        }
        if (processlist2.size() > 0) {
            temp_schedule = MoveNeighborSearchOld(schedule, jobs, jobList, processlist2, flag2);
            if (temp_schedule.get_schedule_id()!=-1) scheduleList.push_back(temp_schedule);
        }
...
Schedule GreedyAdjust(const Schedule &schedule, const std::vector<Job> &jobs, const std::vector<std::string> &jobList) {
    ...
    for (int i = 0; i<schedule_items.size(); i++) {
        for (int j = 1; j<schedule_items[i].process_count;j++) {
            ...
            // std::cout << hasCycleold(graph1) << std::endl;
            if (!hasCycleold(graph1)) {
                std::cout << "old time: " << schedule.get_TotalTime() << ", new time: " << CalculateTotalTime(schedule1) << std::endl;
            }
            if (!hasCycleold(graph1) && CalculateTotalTime(schedule1) < schedule.get_TotalTime()) {
                std::cout << "old time: " << schedule.get_TotalTime() << ", new time: " << CalculateTotalTime(schedule1) << std::endl;
                flag = true;
                break;
            } else {
...
bool hasCycleold(const std::vector<std::vector<int>> &adjMatrix) {  // 判断该邻接链表是否有环，如果有环返回true，否则返回false
...
Schedule ExchangeNeighborSearchOld(const Schedule &schedule, const std::vector<Job> &jobs, const std::vector<std::string> &jobList, std::vector<std::string> &processlist, bool &flag) {
    ...
    for (auto process: processlist) {
        GetMachineIdAndItemIdByProcessOld(schedule_items, process, 1, machine_id, item_id);
        if (item_id>0) {
            ...
            // std::cout << "now graph status: " << hasCycleold(graph1) << std::endl;
            // if (!hasCycleold(graph1)) std::cout << "old time: " << schedule.get_TotalTime() << ", new time: " << CalculateTotalTime(res_schedule) << std::endl;
            if (!hasCycleold(graph1) && CalculateTotalTime(res_schedule) < schedule.get_TotalTime()) {
...
std::vector<std::string> ScoreProcessForExchangeOld(const Schedule &schedule, const std::vector<Job> &jobs, const std::vector<std::string> &jobList) {
        ...
        for (int i = 0; i<jobs.size(); i++) {
            if (job == jobs[i].get_job_name()) {
                for (int j = 0; j<jobs[i].get_process_count(); j++) {
                    if (const int item_index = GetItemIndexOld(schedule, i, j); item_index != -1) {
...
// 获取调度位置编号
int GetItemIndexOld(const Schedule &schedule, const int job_id, const int process_id) {
...
// 获取工序所在的机器和位置编号
void GetMachineIdAndItemIdByProcessOld(const std::vector<Schedule_item> &schedule_items, const std::string &process,int type, int &x, int &y) {
    for (int i=0;i<schedule_items.size();i++) {
...
Schedule MoveNeighborSearchOld(const Schedule &schedule, const std::vector<Job> &jobs, const std::vector<std::string> &jobList, std::vector<std::string> &processlist, bool &flag) {
    Schedule res_schedule = schedule;
    res_schedule.set_schedule_id(-1);
    ...
    std::string temp_process = "";
    for (auto process: processlist) {
        GetMachineIdAndItemIdByProcessOld(schedule_items, process, 2, machine_id, item_id);
        // std::cout << process << ":" << machine_id << " " << item_id << std::endl;
        if (item_id>0) {
            job_id = schedule_items[machine_id].schedule_process[item_id].job_id;
            process_id = schedule_items[machine_id].schedule_process[item_id].process_id;

            Job_process temp_job_process = SelectJobByJobIdOld(jobs, job_id).get_job_process()[process_id];
            ...
            for (int i =0; i < temp_job_process.machine_count; i++) {
                ...
                process_count = GetProcessCountByMachineIdOld(schedule, machine_id_1);
...
int GetDifferFromIdealMachine(const Schedule &schedule, const std::vector<Job> &jobs, const int job_id, const int process_id) {
    ...
    for (int i =0; i < temp_job_process.machine_count; i++) {
        ...
        process_count = GetProcessCountByMachineIdOld(schedule, machine_id);
        if (process_count < min_process_count) {
            min_process_count = process_count;
        }
    }
...
// 获取机器上的工序数量
int GetProcessCountByMachineIdOld(const Schedule &schedule, const int machine_id) {
    for (int i = 0; i<schedule.get_schedule_items().size(); i++) {
...
// 根据工件编号，获取工件
Job SelectJobByJobIdOld(const std::vector<Job> &jobs, const int job_id) {
    for (auto job: jobs) {
